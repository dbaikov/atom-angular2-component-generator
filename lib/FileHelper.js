...
var FileHelper = (function () {
    ...
    ;
    FileHelper.createSpec = function (componentDir, componentName, config) {
        var templateFileName = this.assetRootDir + '/templates/spec.template';
        if (config.template) {
            templateFileName = this.resolveWorkspaceRoot(config.template);
        }
        var specContent = fs.readFileSync(templateFileName).toString()
            .replace(/{componentName}/g, componentName)
            .replace(/{className}/g, changeCase.pascalCase(componentName));
        var filename = componentDir + "/" + componentName + ".component." + config.extension;
        if (config.create) {
